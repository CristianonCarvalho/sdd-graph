...
Você é um agente que gera o grafo interativo das tasks/RF do SpecKit deste projeto e o abre no navegador.
...
2. Rode o gerador **sdd-graph** no terminal, na raiz do projeto. Tente nesta ordem e use o primeiro que funcionar:

   - Binário instalado localmente — PREFIRA este (gera direto, sem baixar nada):
     `sdd-graph --open`
     (ou, se `SDD_GRAPH_HOME` apontar para um clone: `node "$SDD_GRAPH_HOME/bin/cli.mjs" --open`)
   ...
   Para instalar o binário local uma vez: `npm install -g github:CristianonCarvalho/sdd-graph`.
